...
namespace StrideTerrain.Player
{
    public class PlayerController : SyncScript
    {
        ...
        private void Move(float speed)
        {
            ...
            if (MultiTypeCameraController.CameraType != CameraType.FirstPerson)
            {          // Character orientation
                if (moveDirection.Length() > 0.001)
                {
                    yawOrientation = MathUtil.RadiansToDegrees((float)Math.Atan2(-moveDirection.Z, moveDirection.X) + MathUtil.PiOverTwo);
                }
                modelChildEntity.Transform.Rotation =
                    Quaternion.RotationYawPitchRoll(MathUtil.DegreesToRadians(yawOrientation), 0, 0);
            }
        }
    }
}
